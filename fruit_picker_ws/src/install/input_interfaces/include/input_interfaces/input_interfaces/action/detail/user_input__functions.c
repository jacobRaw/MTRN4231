// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from input_interfaces:action/UserInput.idl
// generated code does not contain a copyright notice
#include "input_interfaces/action/detail/user_input__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


// Include directives for member types
// Member `command`
// Member `parameters`
#include "rosidl_runtime_c/string_functions.h"

bool
input_interfaces__action__UserInput_Goal__init(input_interfaces__action__UserInput_Goal * msg)
{
  if (!msg) {
    return false;
  }
  // command
  if (!rosidl_runtime_c__String__init(&msg->command)) {
    input_interfaces__action__UserInput_Goal__fini(msg);
    return false;
  }
  // parameters
  if (!rosidl_runtime_c__String__init(&msg->parameters)) {
    input_interfaces__action__UserInput_Goal__fini(msg);
    return false;
  }
  return true;
}

void
input_interfaces__action__UserInput_Goal__fini(input_interfaces__action__UserInput_Goal * msg)
{
  if (!msg) {
    return;
  }
  // command
  rosidl_runtime_c__String__fini(&msg->command);
  // parameters
  rosidl_runtime_c__String__fini(&msg->parameters);
}

bool
input_interfaces__action__UserInput_Goal__are_equal(const input_interfaces__action__UserInput_Goal * lhs, const input_interfaces__action__UserInput_Goal * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // command
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->command), &(rhs->command)))
  {
    return false;
  }
  // parameters
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->parameters), &(rhs->parameters)))
  {
    return false;
  }
  return true;
}

bool
input_interfaces__action__UserInput_Goal__copy(
  const input_interfaces__action__UserInput_Goal * input,
  input_interfaces__action__UserInput_Goal * output)
{
  if (!input || !output) {
    return false;
  }
  // command
  if (!rosidl_runtime_c__String__copy(
      &(input->command), &(output->command)))
  {
    return false;
  }
  // parameters
  if (!rosidl_runtime_c__String__copy(
      &(input->parameters), &(output->parameters)))
  {
    return false;
  }
  return true;
}

input_interfaces__action__UserInput_Goal *
input_interfaces__action__UserInput_Goal__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  input_interfaces__action__UserInput_Goal * msg = (input_interfaces__action__UserInput_Goal *)allocator.allocate(sizeof(input_interfaces__action__UserInput_Goal), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(input_interfaces__action__UserInput_Goal));
  bool success = input_interfaces__action__UserInput_Goal__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
input_interfaces__action__UserInput_Goal__destroy(input_interfaces__action__UserInput_Goal * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    input_interfaces__action__UserInput_Goal__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
input_interfaces__action__UserInput_Goal__Sequence__init(input_interfaces__action__UserInput_Goal__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  input_interfaces__action__UserInput_Goal * data = NULL;

  if (size) {
    data = (input_interfaces__action__UserInput_Goal *)allocator.zero_allocate(size, sizeof(input_interfaces__action__UserInput_Goal), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = input_interfaces__action__UserInput_Goal__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        input_interfaces__action__UserInput_Goal__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
input_interfaces__action__UserInput_Goal__Sequence__fini(input_interfaces__action__UserInput_Goal__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      input_interfaces__action__UserInput_Goal__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

input_interfaces__action__UserInput_Goal__Sequence *
input_interfaces__action__UserInput_Goal__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  input_interfaces__action__UserInput_Goal__Sequence * array = (input_interfaces__action__UserInput_Goal__Sequence *)allocator.allocate(sizeof(input_interfaces__action__UserInput_Goal__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = input_interfaces__action__UserInput_Goal__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
input_interfaces__action__UserInput_Goal__Sequence__destroy(input_interfaces__action__UserInput_Goal__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    input_interfaces__action__UserInput_Goal__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
input_interfaces__action__UserInput_Goal__Sequence__are_equal(const input_interfaces__action__UserInput_Goal__Sequence * lhs, const input_interfaces__action__UserInput_Goal__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!input_interfaces__action__UserInput_Goal__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
input_interfaces__action__UserInput_Goal__Sequence__copy(
  const input_interfaces__action__UserInput_Goal__Sequence * input,
  input_interfaces__action__UserInput_Goal__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(input_interfaces__action__UserInput_Goal);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    input_interfaces__action__UserInput_Goal * data =
      (input_interfaces__action__UserInput_Goal *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!input_interfaces__action__UserInput_Goal__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          input_interfaces__action__UserInput_Goal__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!input_interfaces__action__UserInput_Goal__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `state_msg`
// already included above
// #include "rosidl_runtime_c/string_functions.h"

bool
input_interfaces__action__UserInput_Result__init(input_interfaces__action__UserInput_Result * msg)
{
  if (!msg) {
    return false;
  }
  // progress
  // state_msg
  if (!rosidl_runtime_c__String__init(&msg->state_msg)) {
    input_interfaces__action__UserInput_Result__fini(msg);
    return false;
  }
  // success
  return true;
}

void
input_interfaces__action__UserInput_Result__fini(input_interfaces__action__UserInput_Result * msg)
{
  if (!msg) {
    return;
  }
  // progress
  // state_msg
  rosidl_runtime_c__String__fini(&msg->state_msg);
  // success
}

bool
input_interfaces__action__UserInput_Result__are_equal(const input_interfaces__action__UserInput_Result * lhs, const input_interfaces__action__UserInput_Result * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // progress
  if (lhs->progress != rhs->progress) {
    return false;
  }
  // state_msg
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->state_msg), &(rhs->state_msg)))
  {
    return false;
  }
  // success
  if (lhs->success != rhs->success) {
    return false;
  }
  return true;
}

bool
input_interfaces__action__UserInput_Result__copy(
  const input_interfaces__action__UserInput_Result * input,
  input_interfaces__action__UserInput_Result * output)
{
  if (!input || !output) {
    return false;
  }
  // progress
  output->progress = input->progress;
  // state_msg
  if (!rosidl_runtime_c__String__copy(
      &(input->state_msg), &(output->state_msg)))
  {
    return false;
  }
  // success
  output->success = input->success;
  return true;
}

input_interfaces__action__UserInput_Result *
input_interfaces__action__UserInput_Result__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  input_interfaces__action__UserInput_Result * msg = (input_interfaces__action__UserInput_Result *)allocator.allocate(sizeof(input_interfaces__action__UserInput_Result), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(input_interfaces__action__UserInput_Result));
  bool success = input_interfaces__action__UserInput_Result__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
input_interfaces__action__UserInput_Result__destroy(input_interfaces__action__UserInput_Result * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    input_interfaces__action__UserInput_Result__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
input_interfaces__action__UserInput_Result__Sequence__init(input_interfaces__action__UserInput_Result__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  input_interfaces__action__UserInput_Result * data = NULL;

  if (size) {
    data = (input_interfaces__action__UserInput_Result *)allocator.zero_allocate(size, sizeof(input_interfaces__action__UserInput_Result), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = input_interfaces__action__UserInput_Result__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        input_interfaces__action__UserInput_Result__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
input_interfaces__action__UserInput_Result__Sequence__fini(input_interfaces__action__UserInput_Result__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      input_interfaces__action__UserInput_Result__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

input_interfaces__action__UserInput_Result__Sequence *
input_interfaces__action__UserInput_Result__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  input_interfaces__action__UserInput_Result__Sequence * array = (input_interfaces__action__UserInput_Result__Sequence *)allocator.allocate(sizeof(input_interfaces__action__UserInput_Result__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = input_interfaces__action__UserInput_Result__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
input_interfaces__action__UserInput_Result__Sequence__destroy(input_interfaces__action__UserInput_Result__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    input_interfaces__action__UserInput_Result__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
input_interfaces__action__UserInput_Result__Sequence__are_equal(const input_interfaces__action__UserInput_Result__Sequence * lhs, const input_interfaces__action__UserInput_Result__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!input_interfaces__action__UserInput_Result__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
input_interfaces__action__UserInput_Result__Sequence__copy(
  const input_interfaces__action__UserInput_Result__Sequence * input,
  input_interfaces__action__UserInput_Result__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(input_interfaces__action__UserInput_Result);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    input_interfaces__action__UserInput_Result * data =
      (input_interfaces__action__UserInput_Result *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!input_interfaces__action__UserInput_Result__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          input_interfaces__action__UserInput_Result__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!input_interfaces__action__UserInput_Result__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `state_msg`
// already included above
// #include "rosidl_runtime_c/string_functions.h"

bool
input_interfaces__action__UserInput_Feedback__init(input_interfaces__action__UserInput_Feedback * msg)
{
  if (!msg) {
    return false;
  }
  // state_msg
  if (!rosidl_runtime_c__String__init(&msg->state_msg)) {
    input_interfaces__action__UserInput_Feedback__fini(msg);
    return false;
  }
  // progress
  return true;
}

void
input_interfaces__action__UserInput_Feedback__fini(input_interfaces__action__UserInput_Feedback * msg)
{
  if (!msg) {
    return;
  }
  // state_msg
  rosidl_runtime_c__String__fini(&msg->state_msg);
  // progress
}

bool
input_interfaces__action__UserInput_Feedback__are_equal(const input_interfaces__action__UserInput_Feedback * lhs, const input_interfaces__action__UserInput_Feedback * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // state_msg
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->state_msg), &(rhs->state_msg)))
  {
    return false;
  }
  // progress
  if (lhs->progress != rhs->progress) {
    return false;
  }
  return true;
}

bool
input_interfaces__action__UserInput_Feedback__copy(
  const input_interfaces__action__UserInput_Feedback * input,
  input_interfaces__action__UserInput_Feedback * output)
{
  if (!input || !output) {
    return false;
  }
  // state_msg
  if (!rosidl_runtime_c__String__copy(
      &(input->state_msg), &(output->state_msg)))
  {
    return false;
  }
  // progress
  output->progress = input->progress;
  return true;
}

input_interfaces__action__UserInput_Feedback *
input_interfaces__action__UserInput_Feedback__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  input_interfaces__action__UserInput_Feedback * msg = (input_interfaces__action__UserInput_Feedback *)allocator.allocate(sizeof(input_interfaces__action__UserInput_Feedback), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(input_interfaces__action__UserInput_Feedback));
  bool success = input_interfaces__action__UserInput_Feedback__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
input_interfaces__action__UserInput_Feedback__destroy(input_interfaces__action__UserInput_Feedback * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    input_interfaces__action__UserInput_Feedback__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
input_interfaces__action__UserInput_Feedback__Sequence__init(input_interfaces__action__UserInput_Feedback__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  input_interfaces__action__UserInput_Feedback * data = NULL;

  if (size) {
    data = (input_interfaces__action__UserInput_Feedback *)allocator.zero_allocate(size, sizeof(input_interfaces__action__UserInput_Feedback), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = input_interfaces__action__UserInput_Feedback__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        input_interfaces__action__UserInput_Feedback__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
input_interfaces__action__UserInput_Feedback__Sequence__fini(input_interfaces__action__UserInput_Feedback__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      input_interfaces__action__UserInput_Feedback__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

input_interfaces__action__UserInput_Feedback__Sequence *
input_interfaces__action__UserInput_Feedback__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  input_interfaces__action__UserInput_Feedback__Sequence * array = (input_interfaces__action__UserInput_Feedback__Sequence *)allocator.allocate(sizeof(input_interfaces__action__UserInput_Feedback__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = input_interfaces__action__UserInput_Feedback__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
input_interfaces__action__UserInput_Feedback__Sequence__destroy(input_interfaces__action__UserInput_Feedback__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    input_interfaces__action__UserInput_Feedback__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
input_interfaces__action__UserInput_Feedback__Sequence__are_equal(const input_interfaces__action__UserInput_Feedback__Sequence * lhs, const input_interfaces__action__UserInput_Feedback__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!input_interfaces__action__UserInput_Feedback__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
input_interfaces__action__UserInput_Feedback__Sequence__copy(
  const input_interfaces__action__UserInput_Feedback__Sequence * input,
  input_interfaces__action__UserInput_Feedback__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(input_interfaces__action__UserInput_Feedback);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    input_interfaces__action__UserInput_Feedback * data =
      (input_interfaces__action__UserInput_Feedback *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!input_interfaces__action__UserInput_Feedback__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          input_interfaces__action__UserInput_Feedback__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!input_interfaces__action__UserInput_Feedback__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
#include "unique_identifier_msgs/msg/detail/uuid__functions.h"
// Member `goal`
// already included above
// #include "input_interfaces/action/detail/user_input__functions.h"

bool
input_interfaces__action__UserInput_SendGoal_Request__init(input_interfaces__action__UserInput_SendGoal_Request * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    input_interfaces__action__UserInput_SendGoal_Request__fini(msg);
    return false;
  }
  // goal
  if (!input_interfaces__action__UserInput_Goal__init(&msg->goal)) {
    input_interfaces__action__UserInput_SendGoal_Request__fini(msg);
    return false;
  }
  return true;
}

void
input_interfaces__action__UserInput_SendGoal_Request__fini(input_interfaces__action__UserInput_SendGoal_Request * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
  // goal
  input_interfaces__action__UserInput_Goal__fini(&msg->goal);
}

bool
input_interfaces__action__UserInput_SendGoal_Request__are_equal(const input_interfaces__action__UserInput_SendGoal_Request * lhs, const input_interfaces__action__UserInput_SendGoal_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  // goal
  if (!input_interfaces__action__UserInput_Goal__are_equal(
      &(lhs->goal), &(rhs->goal)))
  {
    return false;
  }
  return true;
}

bool
input_interfaces__action__UserInput_SendGoal_Request__copy(
  const input_interfaces__action__UserInput_SendGoal_Request * input,
  input_interfaces__action__UserInput_SendGoal_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  // goal
  if (!input_interfaces__action__UserInput_Goal__copy(
      &(input->goal), &(output->goal)))
  {
    return false;
  }
  return true;
}

input_interfaces__action__UserInput_SendGoal_Request *
input_interfaces__action__UserInput_SendGoal_Request__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  input_interfaces__action__UserInput_SendGoal_Request * msg = (input_interfaces__action__UserInput_SendGoal_Request *)allocator.allocate(sizeof(input_interfaces__action__UserInput_SendGoal_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(input_interfaces__action__UserInput_SendGoal_Request));
  bool success = input_interfaces__action__UserInput_SendGoal_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
input_interfaces__action__UserInput_SendGoal_Request__destroy(input_interfaces__action__UserInput_SendGoal_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    input_interfaces__action__UserInput_SendGoal_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
input_interfaces__action__UserInput_SendGoal_Request__Sequence__init(input_interfaces__action__UserInput_SendGoal_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  input_interfaces__action__UserInput_SendGoal_Request * data = NULL;

  if (size) {
    data = (input_interfaces__action__UserInput_SendGoal_Request *)allocator.zero_allocate(size, sizeof(input_interfaces__action__UserInput_SendGoal_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = input_interfaces__action__UserInput_SendGoal_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        input_interfaces__action__UserInput_SendGoal_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
input_interfaces__action__UserInput_SendGoal_Request__Sequence__fini(input_interfaces__action__UserInput_SendGoal_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      input_interfaces__action__UserInput_SendGoal_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

input_interfaces__action__UserInput_SendGoal_Request__Sequence *
input_interfaces__action__UserInput_SendGoal_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  input_interfaces__action__UserInput_SendGoal_Request__Sequence * array = (input_interfaces__action__UserInput_SendGoal_Request__Sequence *)allocator.allocate(sizeof(input_interfaces__action__UserInput_SendGoal_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = input_interfaces__action__UserInput_SendGoal_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
input_interfaces__action__UserInput_SendGoal_Request__Sequence__destroy(input_interfaces__action__UserInput_SendGoal_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    input_interfaces__action__UserInput_SendGoal_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
input_interfaces__action__UserInput_SendGoal_Request__Sequence__are_equal(const input_interfaces__action__UserInput_SendGoal_Request__Sequence * lhs, const input_interfaces__action__UserInput_SendGoal_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!input_interfaces__action__UserInput_SendGoal_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
input_interfaces__action__UserInput_SendGoal_Request__Sequence__copy(
  const input_interfaces__action__UserInput_SendGoal_Request__Sequence * input,
  input_interfaces__action__UserInput_SendGoal_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(input_interfaces__action__UserInput_SendGoal_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    input_interfaces__action__UserInput_SendGoal_Request * data =
      (input_interfaces__action__UserInput_SendGoal_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!input_interfaces__action__UserInput_SendGoal_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          input_interfaces__action__UserInput_SendGoal_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!input_interfaces__action__UserInput_SendGoal_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `stamp`
#include "builtin_interfaces/msg/detail/time__functions.h"

bool
input_interfaces__action__UserInput_SendGoal_Response__init(input_interfaces__action__UserInput_SendGoal_Response * msg)
{
  if (!msg) {
    return false;
  }
  // accepted
  // stamp
  if (!builtin_interfaces__msg__Time__init(&msg->stamp)) {
    input_interfaces__action__UserInput_SendGoal_Response__fini(msg);
    return false;
  }
  return true;
}

void
input_interfaces__action__UserInput_SendGoal_Response__fini(input_interfaces__action__UserInput_SendGoal_Response * msg)
{
  if (!msg) {
    return;
  }
  // accepted
  // stamp
  builtin_interfaces__msg__Time__fini(&msg->stamp);
}

bool
input_interfaces__action__UserInput_SendGoal_Response__are_equal(const input_interfaces__action__UserInput_SendGoal_Response * lhs, const input_interfaces__action__UserInput_SendGoal_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // accepted
  if (lhs->accepted != rhs->accepted) {
    return false;
  }
  // stamp
  if (!builtin_interfaces__msg__Time__are_equal(
      &(lhs->stamp), &(rhs->stamp)))
  {
    return false;
  }
  return true;
}

bool
input_interfaces__action__UserInput_SendGoal_Response__copy(
  const input_interfaces__action__UserInput_SendGoal_Response * input,
  input_interfaces__action__UserInput_SendGoal_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // accepted
  output->accepted = input->accepted;
  // stamp
  if (!builtin_interfaces__msg__Time__copy(
      &(input->stamp), &(output->stamp)))
  {
    return false;
  }
  return true;
}

input_interfaces__action__UserInput_SendGoal_Response *
input_interfaces__action__UserInput_SendGoal_Response__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  input_interfaces__action__UserInput_SendGoal_Response * msg = (input_interfaces__action__UserInput_SendGoal_Response *)allocator.allocate(sizeof(input_interfaces__action__UserInput_SendGoal_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(input_interfaces__action__UserInput_SendGoal_Response));
  bool success = input_interfaces__action__UserInput_SendGoal_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
input_interfaces__action__UserInput_SendGoal_Response__destroy(input_interfaces__action__UserInput_SendGoal_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    input_interfaces__action__UserInput_SendGoal_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
input_interfaces__action__UserInput_SendGoal_Response__Sequence__init(input_interfaces__action__UserInput_SendGoal_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  input_interfaces__action__UserInput_SendGoal_Response * data = NULL;

  if (size) {
    data = (input_interfaces__action__UserInput_SendGoal_Response *)allocator.zero_allocate(size, sizeof(input_interfaces__action__UserInput_SendGoal_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = input_interfaces__action__UserInput_SendGoal_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        input_interfaces__action__UserInput_SendGoal_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
input_interfaces__action__UserInput_SendGoal_Response__Sequence__fini(input_interfaces__action__UserInput_SendGoal_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      input_interfaces__action__UserInput_SendGoal_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

input_interfaces__action__UserInput_SendGoal_Response__Sequence *
input_interfaces__action__UserInput_SendGoal_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  input_interfaces__action__UserInput_SendGoal_Response__Sequence * array = (input_interfaces__action__UserInput_SendGoal_Response__Sequence *)allocator.allocate(sizeof(input_interfaces__action__UserInput_SendGoal_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = input_interfaces__action__UserInput_SendGoal_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
input_interfaces__action__UserInput_SendGoal_Response__Sequence__destroy(input_interfaces__action__UserInput_SendGoal_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    input_interfaces__action__UserInput_SendGoal_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
input_interfaces__action__UserInput_SendGoal_Response__Sequence__are_equal(const input_interfaces__action__UserInput_SendGoal_Response__Sequence * lhs, const input_interfaces__action__UserInput_SendGoal_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!input_interfaces__action__UserInput_SendGoal_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
input_interfaces__action__UserInput_SendGoal_Response__Sequence__copy(
  const input_interfaces__action__UserInput_SendGoal_Response__Sequence * input,
  input_interfaces__action__UserInput_SendGoal_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(input_interfaces__action__UserInput_SendGoal_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    input_interfaces__action__UserInput_SendGoal_Response * data =
      (input_interfaces__action__UserInput_SendGoal_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!input_interfaces__action__UserInput_SendGoal_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          input_interfaces__action__UserInput_SendGoal_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!input_interfaces__action__UserInput_SendGoal_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
// already included above
// #include "unique_identifier_msgs/msg/detail/uuid__functions.h"

bool
input_interfaces__action__UserInput_GetResult_Request__init(input_interfaces__action__UserInput_GetResult_Request * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    input_interfaces__action__UserInput_GetResult_Request__fini(msg);
    return false;
  }
  return true;
}

void
input_interfaces__action__UserInput_GetResult_Request__fini(input_interfaces__action__UserInput_GetResult_Request * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
}

bool
input_interfaces__action__UserInput_GetResult_Request__are_equal(const input_interfaces__action__UserInput_GetResult_Request * lhs, const input_interfaces__action__UserInput_GetResult_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  return true;
}

bool
input_interfaces__action__UserInput_GetResult_Request__copy(
  const input_interfaces__action__UserInput_GetResult_Request * input,
  input_interfaces__action__UserInput_GetResult_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  return true;
}

input_interfaces__action__UserInput_GetResult_Request *
input_interfaces__action__UserInput_GetResult_Request__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  input_interfaces__action__UserInput_GetResult_Request * msg = (input_interfaces__action__UserInput_GetResult_Request *)allocator.allocate(sizeof(input_interfaces__action__UserInput_GetResult_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(input_interfaces__action__UserInput_GetResult_Request));
  bool success = input_interfaces__action__UserInput_GetResult_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
input_interfaces__action__UserInput_GetResult_Request__destroy(input_interfaces__action__UserInput_GetResult_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    input_interfaces__action__UserInput_GetResult_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
input_interfaces__action__UserInput_GetResult_Request__Sequence__init(input_interfaces__action__UserInput_GetResult_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  input_interfaces__action__UserInput_GetResult_Request * data = NULL;

  if (size) {
    data = (input_interfaces__action__UserInput_GetResult_Request *)allocator.zero_allocate(size, sizeof(input_interfaces__action__UserInput_GetResult_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = input_interfaces__action__UserInput_GetResult_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        input_interfaces__action__UserInput_GetResult_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
input_interfaces__action__UserInput_GetResult_Request__Sequence__fini(input_interfaces__action__UserInput_GetResult_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      input_interfaces__action__UserInput_GetResult_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

input_interfaces__action__UserInput_GetResult_Request__Sequence *
input_interfaces__action__UserInput_GetResult_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  input_interfaces__action__UserInput_GetResult_Request__Sequence * array = (input_interfaces__action__UserInput_GetResult_Request__Sequence *)allocator.allocate(sizeof(input_interfaces__action__UserInput_GetResult_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = input_interfaces__action__UserInput_GetResult_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
input_interfaces__action__UserInput_GetResult_Request__Sequence__destroy(input_interfaces__action__UserInput_GetResult_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    input_interfaces__action__UserInput_GetResult_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
input_interfaces__action__UserInput_GetResult_Request__Sequence__are_equal(const input_interfaces__action__UserInput_GetResult_Request__Sequence * lhs, const input_interfaces__action__UserInput_GetResult_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!input_interfaces__action__UserInput_GetResult_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
input_interfaces__action__UserInput_GetResult_Request__Sequence__copy(
  const input_interfaces__action__UserInput_GetResult_Request__Sequence * input,
  input_interfaces__action__UserInput_GetResult_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(input_interfaces__action__UserInput_GetResult_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    input_interfaces__action__UserInput_GetResult_Request * data =
      (input_interfaces__action__UserInput_GetResult_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!input_interfaces__action__UserInput_GetResult_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          input_interfaces__action__UserInput_GetResult_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!input_interfaces__action__UserInput_GetResult_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `result`
// already included above
// #include "input_interfaces/action/detail/user_input__functions.h"

bool
input_interfaces__action__UserInput_GetResult_Response__init(input_interfaces__action__UserInput_GetResult_Response * msg)
{
  if (!msg) {
    return false;
  }
  // status
  // result
  if (!input_interfaces__action__UserInput_Result__init(&msg->result)) {
    input_interfaces__action__UserInput_GetResult_Response__fini(msg);
    return false;
  }
  return true;
}

void
input_interfaces__action__UserInput_GetResult_Response__fini(input_interfaces__action__UserInput_GetResult_Response * msg)
{
  if (!msg) {
    return;
  }
  // status
  // result
  input_interfaces__action__UserInput_Result__fini(&msg->result);
}

bool
input_interfaces__action__UserInput_GetResult_Response__are_equal(const input_interfaces__action__UserInput_GetResult_Response * lhs, const input_interfaces__action__UserInput_GetResult_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // status
  if (lhs->status != rhs->status) {
    return false;
  }
  // result
  if (!input_interfaces__action__UserInput_Result__are_equal(
      &(lhs->result), &(rhs->result)))
  {
    return false;
  }
  return true;
}

bool
input_interfaces__action__UserInput_GetResult_Response__copy(
  const input_interfaces__action__UserInput_GetResult_Response * input,
  input_interfaces__action__UserInput_GetResult_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // status
  output->status = input->status;
  // result
  if (!input_interfaces__action__UserInput_Result__copy(
      &(input->result), &(output->result)))
  {
    return false;
  }
  return true;
}

input_interfaces__action__UserInput_GetResult_Response *
input_interfaces__action__UserInput_GetResult_Response__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  input_interfaces__action__UserInput_GetResult_Response * msg = (input_interfaces__action__UserInput_GetResult_Response *)allocator.allocate(sizeof(input_interfaces__action__UserInput_GetResult_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(input_interfaces__action__UserInput_GetResult_Response));
  bool success = input_interfaces__action__UserInput_GetResult_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
input_interfaces__action__UserInput_GetResult_Response__destroy(input_interfaces__action__UserInput_GetResult_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    input_interfaces__action__UserInput_GetResult_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
input_interfaces__action__UserInput_GetResult_Response__Sequence__init(input_interfaces__action__UserInput_GetResult_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  input_interfaces__action__UserInput_GetResult_Response * data = NULL;

  if (size) {
    data = (input_interfaces__action__UserInput_GetResult_Response *)allocator.zero_allocate(size, sizeof(input_interfaces__action__UserInput_GetResult_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = input_interfaces__action__UserInput_GetResult_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        input_interfaces__action__UserInput_GetResult_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
input_interfaces__action__UserInput_GetResult_Response__Sequence__fini(input_interfaces__action__UserInput_GetResult_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      input_interfaces__action__UserInput_GetResult_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

input_interfaces__action__UserInput_GetResult_Response__Sequence *
input_interfaces__action__UserInput_GetResult_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  input_interfaces__action__UserInput_GetResult_Response__Sequence * array = (input_interfaces__action__UserInput_GetResult_Response__Sequence *)allocator.allocate(sizeof(input_interfaces__action__UserInput_GetResult_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = input_interfaces__action__UserInput_GetResult_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
input_interfaces__action__UserInput_GetResult_Response__Sequence__destroy(input_interfaces__action__UserInput_GetResult_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    input_interfaces__action__UserInput_GetResult_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
input_interfaces__action__UserInput_GetResult_Response__Sequence__are_equal(const input_interfaces__action__UserInput_GetResult_Response__Sequence * lhs, const input_interfaces__action__UserInput_GetResult_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!input_interfaces__action__UserInput_GetResult_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
input_interfaces__action__UserInput_GetResult_Response__Sequence__copy(
  const input_interfaces__action__UserInput_GetResult_Response__Sequence * input,
  input_interfaces__action__UserInput_GetResult_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(input_interfaces__action__UserInput_GetResult_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    input_interfaces__action__UserInput_GetResult_Response * data =
      (input_interfaces__action__UserInput_GetResult_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!input_interfaces__action__UserInput_GetResult_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          input_interfaces__action__UserInput_GetResult_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!input_interfaces__action__UserInput_GetResult_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
// already included above
// #include "unique_identifier_msgs/msg/detail/uuid__functions.h"
// Member `feedback`
// already included above
// #include "input_interfaces/action/detail/user_input__functions.h"

bool
input_interfaces__action__UserInput_FeedbackMessage__init(input_interfaces__action__UserInput_FeedbackMessage * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    input_interfaces__action__UserInput_FeedbackMessage__fini(msg);
    return false;
  }
  // feedback
  if (!input_interfaces__action__UserInput_Feedback__init(&msg->feedback)) {
    input_interfaces__action__UserInput_FeedbackMessage__fini(msg);
    return false;
  }
  return true;
}

void
input_interfaces__action__UserInput_FeedbackMessage__fini(input_interfaces__action__UserInput_FeedbackMessage * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
  // feedback
  input_interfaces__action__UserInput_Feedback__fini(&msg->feedback);
}

bool
input_interfaces__action__UserInput_FeedbackMessage__are_equal(const input_interfaces__action__UserInput_FeedbackMessage * lhs, const input_interfaces__action__UserInput_FeedbackMessage * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  // feedback
  if (!input_interfaces__action__UserInput_Feedback__are_equal(
      &(lhs->feedback), &(rhs->feedback)))
  {
    return false;
  }
  return true;
}

bool
input_interfaces__action__UserInput_FeedbackMessage__copy(
  const input_interfaces__action__UserInput_FeedbackMessage * input,
  input_interfaces__action__UserInput_FeedbackMessage * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  // feedback
  if (!input_interfaces__action__UserInput_Feedback__copy(
      &(input->feedback), &(output->feedback)))
  {
    return false;
  }
  return true;
}

input_interfaces__action__UserInput_FeedbackMessage *
input_interfaces__action__UserInput_FeedbackMessage__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  input_interfaces__action__UserInput_FeedbackMessage * msg = (input_interfaces__action__UserInput_FeedbackMessage *)allocator.allocate(sizeof(input_interfaces__action__UserInput_FeedbackMessage), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(input_interfaces__action__UserInput_FeedbackMessage));
  bool success = input_interfaces__action__UserInput_FeedbackMessage__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
input_interfaces__action__UserInput_FeedbackMessage__destroy(input_interfaces__action__UserInput_FeedbackMessage * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    input_interfaces__action__UserInput_FeedbackMessage__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
input_interfaces__action__UserInput_FeedbackMessage__Sequence__init(input_interfaces__action__UserInput_FeedbackMessage__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  input_interfaces__action__UserInput_FeedbackMessage * data = NULL;

  if (size) {
    data = (input_interfaces__action__UserInput_FeedbackMessage *)allocator.zero_allocate(size, sizeof(input_interfaces__action__UserInput_FeedbackMessage), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = input_interfaces__action__UserInput_FeedbackMessage__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        input_interfaces__action__UserInput_FeedbackMessage__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
input_interfaces__action__UserInput_FeedbackMessage__Sequence__fini(input_interfaces__action__UserInput_FeedbackMessage__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      input_interfaces__action__UserInput_FeedbackMessage__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

input_interfaces__action__UserInput_FeedbackMessage__Sequence *
input_interfaces__action__UserInput_FeedbackMessage__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  input_interfaces__action__UserInput_FeedbackMessage__Sequence * array = (input_interfaces__action__UserInput_FeedbackMessage__Sequence *)allocator.allocate(sizeof(input_interfaces__action__UserInput_FeedbackMessage__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = input_interfaces__action__UserInput_FeedbackMessage__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
input_interfaces__action__UserInput_FeedbackMessage__Sequence__destroy(input_interfaces__action__UserInput_FeedbackMessage__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    input_interfaces__action__UserInput_FeedbackMessage__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
input_interfaces__action__UserInput_FeedbackMessage__Sequence__are_equal(const input_interfaces__action__UserInput_FeedbackMessage__Sequence * lhs, const input_interfaces__action__UserInput_FeedbackMessage__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!input_interfaces__action__UserInput_FeedbackMessage__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
input_interfaces__action__UserInput_FeedbackMessage__Sequence__copy(
  const input_interfaces__action__UserInput_FeedbackMessage__Sequence * input,
  input_interfaces__action__UserInput_FeedbackMessage__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(input_interfaces__action__UserInput_FeedbackMessage);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    input_interfaces__action__UserInput_FeedbackMessage * data =
      (input_interfaces__action__UserInput_FeedbackMessage *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!input_interfaces__action__UserInput_FeedbackMessage__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          input_interfaces__action__UserInput_FeedbackMessage__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!input_interfaces__action__UserInput_FeedbackMessage__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
